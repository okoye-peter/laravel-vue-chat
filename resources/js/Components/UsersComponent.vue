<template>
  <div class="shadow-2xl h-screen fixed bg-gray-800">
    ...
    <ul class="overflow-y-auto overflow-x-hidden">
        <li class="mb-4 px-4 relative py-3"  v-for="(friend, index) in friends" :key="index" @click="chooseActiveUser(friend.id)">
            <div class="flex items-center">
              ...
              <div class="text-sm  text-right flex-1">
                <aside class="rounded-full bg-green-100 text-green-800 inline-block text-center text-xs" v-show="friend.unread">{{ friend.unread }}</aside>
              </div>
            </div>
        </li>
    </ul>
